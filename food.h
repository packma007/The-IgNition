...
namespace domains {
    ...
    // 음식 이름을 비교하기 좋게 다듬는다.
    // 앞뒤/가운데 공백을 모두 없애고, 영문은 소문자로.
    // "제육 덮밥" 과 "제육덮밥" 을 같은 것으로 보기 위해서다.
    //
    // 공공 DB 어댑터(foodcsv.h)도 같은 규칙으로 맞춰야 하므로 여기서 공개한다 -
    // 두 곳이 각자 다듬으면 로컬에서 찾히던 이름이 공공 DB 에서 안 찾히게 된다.
    std::string foodKey(const std::string& name);
